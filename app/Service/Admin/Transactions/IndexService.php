...
class IndexService
{
    ...
    private function prepareListing($request, $actionData)
    {
        ...
        $json_data = array(
            ...
            "totalCommission" =>intval($queryData['totalCommission']),
            "data"            => $data
        );
...
$totalCommission = null;
        if($request->input("payment_id"))
        {
            $payment = PaymentHistory::where('id', $request->input("payment_id"))->first();
             $transaction = Transaction::with('advertiser:id,sid,name')
    ->select(DB::raw('SUM(commission_amount) as total_commission'))
    ->whereIn('transaction_id', $payment->transaction_idz)
    ->first(); // ✅ returns a single model (stdClass)

$totalCommission = $transaction->total_commission ?? 0;
